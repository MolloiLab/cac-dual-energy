...
calcium_densities = [
	[73, 73, 73, 59, 59, 59, 52, 52, 52],
	[36, 36, 36, 29, 29, 29, 26, 26, 26],
...
begin
	param_base_pth = datadir("calibration_params_low_density/")
	small_pth = string(param_base_pth,"Small.csv")
	med_pth = string(param_base_pth,"Small.csv")
	large_pth = string(param_base_pth,"Small.csv")
...
begin
	...
	for _size in sizes_folders 
		for density in densities
			...
			if (_size == sizes[1])
				_SIZE = "small"
			elseif (_size == sizes[2])
				_SIZE = "medium"
			elseif (_size == sizes[3])
				_SIZE = "large"
			end
			...
			pth = datadir("dcms_validation_low_density/", _size, density, string(energies[1]))
			...
			## High energy
			pth2 = datadir("dcms_validation_low_density/", _size, density, string(energies[2]))
			...
			if (density == densities[1]) && (_size == "large" || _size == "medium" || _size == "small")
				calcium_density = calcium_densities[1]
			
			elseif (density == densities[2]) && (_size == "large" || _size == "medium" || _size == "small")
				calcium_density = calcium_densities[2]
			
			elseif (density == densities[3]) && (_size == "large" || _size == "medium" || _size == "small")
...
begin
    ...
    output_path = string(datadir("results"),"masses_low_density.csv")
