...
Dual-energy computed tomography (DECT) is a promising technology that has been shown to improve the visualization of calcified plaque components and enhance plaque assessment [CITE]. DECT exploits the fact that different tissues have different mass attenuation coefficients when interacting with X-rays of different energies [CITE]. DECT, therefore, allows for material decomposition, which can be potentially be used to directly quantify the calcium mass in DECT scans.

In this paper, we investigate a new calcium mass quantification technique based on dual energy material decompositoion. We compare this method and Agatston scoring against the ground truth calcium mass, in simulated calcification phantoms.
"""
